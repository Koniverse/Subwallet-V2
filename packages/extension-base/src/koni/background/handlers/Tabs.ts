...
import { canDerive, getEVMChainInfo, stripUrl, transformAccountFromPair } from '@subwallet/extension-base/utils';
...
export default class KoniTabs {
  ...
  private async bytesSign (url: string, request: SignerPayloadRaw): Promise<ResponseSigning> {
    ...
    return this.#koniState.sign(url, new RequestBytesSign(request), transformAccountFromPair(pair));
  }
  ...
  private async extrinsicSign (url: string, request: SignerPayloadJSON): Promise<ResponseSigning> {
    ...
    return this.#koniState.sign(url, new RequestExtrinsicSign(request), transformAccountFromPair(pair));
  }
