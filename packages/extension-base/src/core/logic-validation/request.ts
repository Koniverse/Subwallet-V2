...
export function convertErrorMessage (message_: string, name?: string): string[] {
  ...
  if (message.includes('network is currently not supported')) {
    return [t('This network is not yet supported on SubWallet. Import the network on SubWallet and try again'), t('Network not supported')];
  }

  // Authentication
  if (message.includes('Not found address to sign') ||
     message.includes('Unable to find account')) {
  ...
  // Transaction
  if (message.includes('is not a number') || message.includes('invalid number value')) {
    return [t('Amount must be a number. Enter a number and try again'), t('Invalid amount')];
  }

  if (message.includes('calculate estimate gas fee') || message.includes('invalidcode')) {
    return [t('Gas calculation error\n' +
      'Unable to calculate estimated gas for this transaction. Try again or contact support at agent@subwallet.app'), t('Gas calculation error')];
  }
