...
import { DeriveInfo, NextDerivePair } from '@subwallet/extension-base/types';
...
export const findNextDerivePair = (parentAddress: string): NextDerivePair => {
  ...
  const childrenMetadata = children.map(({ meta: { suri },
    type }) => getDerivationInfo(type, suri as string | undefined)).sort((a, b) => {
